...
export let size = CurrentDevice == Device.PC ? Device.PC.size : Device.MOBILE.size;
export let GAME_WIDTH = size * (CurrentDevice == Device.PC ? Device.PC.width : Device.MOBILE.width);
export let GAME_HEIGHT = size * (CurrentDevice == Device.PC ? Device.PC.height : Device.MOBILE.height);
...
function handleFileLoad() {
	//this fn run on each file load
	//best for debugging
	console.log("allFilesLoaded");
}
...
function handleComplete() {
	diceSpriteSheet = new createjs.SpriteSheet({
		...
		frames: {width: 154, height: 158, count: 25, regX: 0, regY: 0, spacing: 0, margin: 0},
		animations: {
			roll: [0, 18, false, 1],
			start: [20, 20],
		},
	...
	ponSelectorSpriteSheet = new createjs.SpriteSheet({
		...
		frames: {width: 78, height: 79, count: 8, regX: 0, regY: 0, spacing: 0, margin: 0},
		animations: {
			rotate: [0, 7, true, 1],
		},
	});
